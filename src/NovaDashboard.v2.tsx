...
// BorisOnboarding archived in src/archive/ — demo opens straight into the app
...
/** Nova Light 2.0 — open demo for recruiters (no password / no Boris welcome). */
...
export default function NovaLightDashboard() {
  ...
  // ── Auth State ────────────────────────────────────
  const auth = useAuth(defaultUser);
  const {
  ...
  // Open demo for recruiters: always authenticated (no password wall)
  const isAuthenticated = DEMO_OPEN_ACCESS ? true : auth.isAuthenticated;
  const setIsAuthenticated = auth.setIsAuthenticated;

  const ALLOWED_USERS = ["Администратор", "Елена Морозова"];

  // Ensure a default user when opening without login
  useEffect(() => {
    if (!DEMO_OPEN_ACCESS) return;
    if (!currentUser) setCurrentUser("Администратор");
  }, [currentUser]);
  ...
  const handleLogout = () => {
    ...
    // Demo stays open — no password wall after "logout"
    if (DEMO_OPEN_ACCESS) {
      setIsAuthenticated(true);
      setCurrentUser(defaultUser || "Администратор");
    } else {
  ...
  // Demo: onboarding welcome disabled (full flow kept in src/archive/)
  ...
  // Password login UI removed in 2.0 (DEMO_OPEN_ACCESS). Full gate lives in src/archive/.
  ...
  return (
    <div className="flex h-screen bg-app text-slate-200">
      ...
      {/* Boris welcome onboarding archived — see src/archive/BorisOnboarding.tsx */}
