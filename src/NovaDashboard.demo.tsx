...
export default function NovaLightDashboard() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    const key = DEMO_AUTO_PASSWORD ? (loginKey || passwordFor(user)) : loginKey;
    ...
    if (expectedKey && key === expectedKey) {
      ...
      setCurrentUser(user);
      setLoginKey(DEMO_AUTO_PASSWORD ? passwordFor(user) : "");
    } else {
  ...
  const handleLogout = () => {
    ...
    setLoginKey(DEMO_AUTO_PASSWORD ? passwordFor(defaultUser || "Администратор") : "");
    setLoginError("");
  };
        ...
        <div className="w-full max-w-lg glass-panel rounded-[32px] border border-white/10 bg-black/50 p-6 sm:p-10 shadow-2xl">
          <div className="mb-8">
            ...
            <p className="text-sm text-slate-400 leading-relaxed">
              Демо для портфолио: выберите роль и нажмите «Войти» — ключ уже подставлен.
            </p>
          </div>
              ...
              {[
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      if (DEMO_AUTO_PASSWORD) setLoginKey(passwordFor(member.name));
                    }}
