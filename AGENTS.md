...
- Frontend: React 18 + Vite + TypeScript + Tailwind, served on port `5173`. Vite proxies `/api` → `http://localhost:3002` (see `vite.config.ts`).
...
- Frontend: `npm run dev` (from repo root). Open `http://localhost:5173/`.
...
- `src/main.tsx` picks a "face" from `VITE_APP_FACE`: default is `v2`, an OPEN DEMO with no login wall — `http://localhost:5173/` loads straight into the dashboard. `VITE_APP_FACE=demo` (`npm run dev:demo`) shows the recruiter login screen instead.
